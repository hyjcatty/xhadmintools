...
$(document).ready(function() {
    ...
    $("#newQRCodeZipModalShow").on('click',function(){


        modal_middle($('#newQRCodeZipModal'));
        $('#newQRCodeZipModal').modal('show') ;
    });
    $("#newQRCodeZipCommit").on('click',function(){
        submit_new_QRCode_zip_module();
    });

    $(window).resize();

});
function QRCode_Zip_Management(){
    clear_window();
    write_title("工厂二维码申请","");
    $("#QRCodeManageView").css("display","block");
...
function GetZipFileList(){
    ...
    var GetZipFileList_callback= function(result){
        if(result.status == "false"){
            show_expiredModule();
            return;
        }
        $("#QRCodeHistory").empty();
        txt ="<thead> <tr> <th>已生成文件 </th> </tr> </thead> <tbody >";
        filelist = result.ret;
        if(filelist === null) filelist = [];
        for(var i=0;i<filelist.length;i++){
            txt = txt + "<tr> <td   ><a href='"+filelist[i].URL+"'>"+ filelist[i].name+"</a></td> </tr>";
        }
        txt = txt+ "</tbody>";
        $("#QRCodeHistory").append(txt);
    };
    JQ_get(admintools_head,map,GetZipFileList_callback);
}
function submit_new_QRCode_zip_module(){
    var FACCode = $("#FACCode_Input").val();
    var PDCode = $("#PDCode_Input").val();
    var PJCode = $("#PJCode_Input").val();
    var UserCode = $("#UserCode_Input").val();
    var ProductType = $("#ProductType_Input").val();
    var FormalFlag = $("#FormalFlag_Choice").val();
    var ApplyNbr = $("#ApplyNbr_Input").val();
    //console.log("new_usr_name:"+new_usr_name);
    if(FACCode === null || FACCode === ""){
        $("#FACCode_Input").attr("placeholder","工厂代码不能为空");
        $("#FACCode_Input").focus();
        return;
    }
    if(PDCode === null || PDCode === ""){
        $("#PDCode_Input").attr("placeholder","产品代码不能为空");
        $("#PDCode_Input").focus();
        return;
    }
    if(PJCode === null || PJCode === ""){
        $("#PJCode_Input").attr("placeholder","项目代码不能为空");
        $("#PJCode_Input").focus();
        return;
    }
    if(UserCode === null || UserCode === ""){
        $("#UserCode_Input").attr("placeholder","用户代码不能为空");
        $("#UserCode_Input").focus();
        return;
    }
    if(ProductType === null || ProductType === ""){
        $("#ProductType_Input").attr("placeholder","产品类型不能为空");
        $("#ProductType_Input").focus();
        return;
    }
    if(ApplyNbr === null || ApplyNbr === ""){
        $("#ApplyNbr_Input").attr("placeholder","申请数量不能为空");
        $("#ApplyNbr_Input").focus();
        return;
    }
    intApplyNbr = parseInt(ApplyNbr);
    if(isNaN(intApplyNbr)||intApplyNbr<=0||intApplyNbr>100){
        $("#ApplyNbr_Input").attr("placeholder","申请数量必须在0-100之间");
        $("#ApplyNbr_Input").val("100");
        $("#ApplyNbr_Input").focus();
        return;
    }
    var QRCode = {
        'FACCode':FACCode,  //工厂代码
        'PDCode':PDCode,    //产品代码
        'PJCode':PJCode,    //项目代码
        'UserCode':UserCode,  //用户代码
        'ProductType':ProductType,  //产品类型
        'FormalFlag':FormalFlag,   //是否正式标签
        'ApplyNbr':ApplyNbr
    };
    new_QRCode_zip(QRCode);
}
function new_QRCode_zip(QRCode){
    ...
    var new_QRCode_zip_callback = function(result){
        var ret = result.status;
        if(ret == "true"){

            $('#newQRCodeZipModal').modal('hide');
            setTimeout(function(){
                show_alarm_module(false,"生成成功！请从列表下载。",GetZipFileList);},500);
        }else{
            setTimeout(function(){
                show_alarm_module(true,"生成失败！"+result.msg,null);},500);
        }
    };
    JQ_get(admintools_head,map,new_QRCode_zip_callback);
